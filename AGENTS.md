...
- Use collection export when another agent or workflow needs a stable handoff payload.

## Agent handoff pattern

The cleanest agent workflow is:

1. `Tracked Channels` defines the niche source
2. `Browse` finds candidate outliers
3. the agent saves the strongest ones into a collection
4. the agent exports that collection as JSON
5. downstream content-planning agents consume the exported collection

That keeps OpenOutlier focused on discovery and curation, while your other agents handle scripting, hooks, briefs, or production planning.
