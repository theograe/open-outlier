...
export async function registerListRoutes(app: FastifyInstance): Promise<void> {
  ...
  const createListHandler = async (request: { body: unknown }, reply: { code: (statusCode: number) => void }) => {
  ...
  app.post("/api/lists", async (request, reply) => {
    return createListHandler(request, reply);
  });

  app.get("/api/lists", async () => {
    return db.prepare(listSummaryQuery).all();
  });
