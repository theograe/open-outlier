...
export class YoutubeClient {
  ...
  async searchChannels(query: string, limit = 10): Promise<ResolvedChannel[]> {
    ...
    const channelIds = [...new Set((response.items ?? []).map((item) => item.snippet?.channelId).filter(Boolean) as string[])];
    if (channelIds.length === 0) {
      return [];
    }

    const channels = await this.request<{ items?: YoutubeChannel[] }>("channels", {
